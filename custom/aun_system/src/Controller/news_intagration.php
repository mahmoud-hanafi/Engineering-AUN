...
class news_intagration extends  ControllerBase{
  public function handle_news_intagration(){
	...
	//$sql = "SELECT * FROM `int_news` inner join int_pics using(int_id) WHERE `fac_ID` = 02 and Int_id >500098  order by int_id asc " ;
	$sql = "SELECT * FROM `int_news` wHERE `fac_ID` = 02  and Int_id >49202  and int_id NOT in (select int_id from int_pics)";
	//print $sql;exit();
